...
\begin{minipage}[b]{4.75in}   %everything in this minipage will be adjacent, left of the thermometer
  ...
  Grank: \Sexpr{ paste(as.character(ElementNames$g_rank)," - ",as.character(grank_desc[[2]]), sep="")}\\
...
mtype <- 'https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png'
mtype <- 'https://{s}.basemaps.cartocdn.com/rastertiles/voyager_labels_under/{z}/{x}/{y}{r}.png'
 
basetiles <- read_osm(bbox, type = 'esri-topo')#, zoom = 8, type = 'esri-topo') #, ext = 1.01) #type = mtype, ext = 1.1) 
# plot it
qtm(basetiles) + 
  tm_shape(ras) + 
    tm_raster(palette = clrs, title = "modeled suitability",alpha = 0.9,
...
\begin{itemize}
  ...
  \item{The repository version (most recent commit SHA-1 hash) used for this run was: \Sexpr{modelrun_meta_data$repo_head}}
