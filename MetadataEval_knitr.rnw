...
Species habitat model for \Sexpr{as.character(ElementNames$CommName)} (\textit{\Sexpr{as.character(ElementNames$SciName)}}).
}
\rfoot{p. \thepage}
...
\begin{minipage}[b]{4.75in}   %everything in this minipage will be adjacent, left of the thermometer
  ...
  \normalsize Species Habitat Model (SHM) assessment metrics and metadata \\
...
par(mar=c(3.2,2.5,.25,0.2),   #bottom, left, top, right
    tcl=-0.1,   #tic length
...
All SHMs are sensitive to data inputs and methodological choices. Table 4 presents scoring of modeling factors based on the model evaluation rubric presented in Sofaer et al. 2019 {\protect\NoHyper\cite{SofaerEtAl2019}\protect\endNoHyper}. \Sexpr{as.character(project_blurb)}
\\
\smallskip
\pagebreak
...
#qtm(basetiles) + 
  ...
  tm_raster(palette = clrs, title = "modeled suitability",alpha = 0.9, style = "fixed", breaks = brks,
            labels = c("Low Habitat Suitability", rep(" ", nclr-2), "High Habitat Suitability"), interpolate = TRUE) + 
  tm_shape(referenceBoundaries) +
  tm_borders(col = "grey", lwd = 1) + 
  tm_shape(studyAreaExtent) + 
  tm_borders(col = "red", lwd = 2) + 
  tm_compass(north = 0, type = "arrow", position = c("left","center")) + 
  tm_scale_bar() +
...
\hangindent=0.5cm Please cite this document and its associated SHM as: \\
Virginia Natural Heritage Program. \Sexpr{format(Sys.Date(), "%Y")}. Species habitat model for \Sexpr{as.character(ElementNames$CommName)} (\textit{\Sexpr{as.character(ElementNames$SciName)}}). Created on \Sexpr{format(Sys.Date(), "%d %b %Y")}. Virginia Department of Conservation and Recreation - Division of Natural Heritage, Richmond, VA.
\end{minipage}
}
