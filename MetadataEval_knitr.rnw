...
basetiles <- NULL
try(basetiles <- read_osm(bbox, type = 'esri-topo', ext = 1.1), silent = T) #,type = mtype, ext = 1.1) 
if (is.null(basetiles)) basetiles <- read_osm(bbox, ext = 1.1)
# try(basetiles <- read_osm(bbox, type = mtype, ext = 1.1))

# rasterize prediction
ras <- raster(ext = extent(basetiles), crs = st_crs(basetiles)$proj4string, res = c(1000,1000))
suppressMessages(ras <- nhSDM::nh_rasterize(results_shape, ras, results_shape$prbblty))
...
qtm(basetiles) +   
  ...
  tm_shape(ras) +
    tm_raster(palette = clrs, title = "modeled suitability",alpha = 0.9, style = "equal",
  ...
  tm_scale_bar() +
  tm_layout(legend.bg.color = "grey40", legend.bg.alpha = 0.3, frame.lwd = 2, outer.margins = c(0,0,0,0))
...
\begin{itemize}
  ...
  \item{Validation metrics requiring a threshold use maximum of sensitivity plus specificity (MaxSSS)}
